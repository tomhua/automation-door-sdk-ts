...
export class SerialPortDriver {
     ...
     */
    getDoors() {
        return commandList.filter((cmd) => cmd.category === 'door');
    }
    /**
     * 获取所有黄灯设备的命令
     * @returns 黄灯设备命令列表
     */
    getYellowLamps() {
        return commandList.filter((cmd) => cmd.category === 'lamp' && cmd.color === 'yellow');
    }
    /**
     * 获取所有绿灯设备的命令
     * @returns 绿灯设备命令列表
     */
    getGreenLamps() {
        return commandList.filter((cmd) => cmd.category === 'lamp' && cmd.color === 'green');
    }
    /**
     * 向多个设备发送命令
     * @param items 设备命令列表
     ...
     * @returns Promise
     */
    async sendToAll(items, commandType) {
